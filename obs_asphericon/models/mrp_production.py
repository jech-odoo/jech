...
class MRPProdction(models.Model):
    ...
    date_deadline = fields.Datetime(
        'Deadline', copy=False, readonly=True, compute='_compute_deadline_date',
        help="Informative date allowing to define when the manufacturing order should be processed at the latest to fulfill delivery on time.")

    restrict_partner_id = fields.Many2one(
        'res.partner', string="Owner", copy=False)
    ...
    def onchange_product_id(self):
        ...
        if self._context.get('default_analytic_id'):
            ...
            self.analytic_account_id = self.env["account.analytic.default"].search(
            [('product_id', '=', self.product_id.id), ('product_id', '!=', False)], limit=1).analytic_id.id
        ...
        for production in self:
            if not production.origin:
                if production.date_planned_start:
                    production.date_deadline = production.date_planned_start + \
    ...
    def _generate_backorder_productions(self, close_mo=True):
        ...
        mail_activities = self.env["mail.activity"].search([('res_model', '=', 'mrp.production'), ('res_id', '=', current_mo.id)])
        if mail_activities:
            mail_activities.write({"res_id": res.id})
        messages = self.env["mail.message"].search([("message_type", "in", ("comment", "notification")), ("model", "=", "mrp.production"), ('res_id', '=', current_mo.id)])
        if messages:
