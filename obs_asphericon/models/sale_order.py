...
class SaleOrder(models.Model):
    ...
    asph_project_ids = fields.Many2many(
        "project.project", string="ASPH Projects", compute="_compute_asph_project_ids"
    )
            ...
            projects = self.env["project.project"].search(
                [
                    (
                        "analytic_account_id",
                        "in",
                        order.order_line.mapped("account_analytic_id").ids,
                    )
                ]
            )
    ...
    def update_prices(self):
        ...
        for line in self.order_line.filtered(
            ...
            and not line.product_id.ignore_sales_price
        ):
            ...
            if self.pricelist_id.discount_policy == "without_discount" and price_unit:
                price_discount_unrounded = self.pricelist_id.get_product_price(
                    product,
                ...
                discount = max(
                    0, (price_unit - price_discount_unrounded) * 100 / price_unit
                )
            else:
                discount = 0
    ...
    def action_view_project_ids(self):
        ...
        res["domain"] = [
            ...
            ("id", "in", self.asph_project_ids.ids),
        ]
        return res
...
class SaleOrderLine(models.Model):
    ...
    reserved_quantities = fields.Float(
        string="Reserved", compute="_compute_reserved_quantities", store=True
    )
    ...
    def product_uom_change(self):
        ...
        if (
            ...
            and not self.product_id.ignore_sales_price
        ):
