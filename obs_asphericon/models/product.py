...
class ProductTemplate(models.Model):
    ...
    ignore_sales_price = fields.Boolean(
        string="Ignore Sales Prices", related="categ_id.ignore_sales_price"
    )
    ignore_vendor_pricelists = fields.Boolean(
        string="Ignore Vendor Pricelists", related="categ_id.ignore_vendor_pricelists"
    )
    ...
    def _compute_analytic_id(self):
        ...
        res = self.env["account.analytic.account"]._get_analytic_account(
    ...
    def _inverse_analytic_id(self):
        for product in self:
            self.env["ir.property"]._set_multi(
                "analytic_id", product._name, {product.id: product.analytic_id}
            )
    ...
    def write(self, vals):
        if "analytic_id" in vals:
            ...
            for product in self:
                AnalyticDefault.search(
                    [
                        ("analytic_id", "=", product.product_variant_id.analytic_id.id),
    ...
    def write(self, vals):
        if "analytic_id" in vals:
            ...
            for product in self:
                AnalyticDefault.search(
                    [
                        ("analytic_id", "=", product.analytic_id.id),
                        ("product_id", "=", product.product_variant_id.id),
                        ("is_from_product", "=", True),
                    ],
                    limit=1,
