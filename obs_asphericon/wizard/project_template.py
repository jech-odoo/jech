...
class ProjectWizard(models.TransientModel):
    ...
    project_manager_id = fields.Many2one("res.users", string="Project Manager", default=lambda self: self.env.uid)
    ...
    def action_create_project_template(self):
        ...
        # if Project is selected  then it will copy the selected project to create a new record with default vals
            ...
            project = self.project_id.copy({'name': self.name, 'user_id': self.project_manager_id.id})
        ...
        active_model, active_id = context.get(
            'active_model'), context.get('active_id')
